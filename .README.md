...
zedu-api-automation/
├── tests/
│   ├── auth.test.js           # Register, Login, Logout, Change Password tests
│   ├── users.test.js          # User management endpoint tests
│   └── organisations.test.js  # Organisation creation tests
├── utils/
│   └── auth.js                # Shared auth utility (getToken) for tests that requires token
├── .env.example               # Environment variable template
├── .gitignore                 # Files excluded from Git
├── babel.config.js            # Babel configuration for Jest ESM support
...
```bash
git clone https://github.com/yourusername/zedu-api-automation.git
...
TEST_USER_PASSWORD=password_used_for_test_users
```
...
`dotenv` | "^17.4.2 | Environment variable management 
 ...
 `babel-jest` | "@babel/core": "^30.3.0"|Jest Babel transformer
